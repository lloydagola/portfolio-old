          ...
          <div data-aos="zoom-in" class="lg:grid lg:grid-cols-12">
             <div class="mt-8 px-5 sm:mt-16 lg:mt-0 lg:col-span-6">
              <div class="flex-shrink-0 w-full sm:mx-auto flex flex-1 items-center justify-center rounded-lg sm:overflow-hidden">
                <img class="rounded-md h-80" src="/images/software-testing-696x392.jpg" :alt="$config.name">
              </div>
            </div>
            <div class="px-4 sm:px-6 lg:border-r lg:border-dashed lg:border-gray-700 sm:text-center md:max-w-2xl md:mx-auto lg:col-span-6 lg:text-left lg:flex lg:items-center">
              <div>
                <nuxt-link v-if="$config.blog.enabled" :to="localePath('/projects')" class="group inline-flex hover:bg-black items-center text-white bg-gray-900 rounded-full p-0.5 pr-2 sm:text-base lg:text-sm xl:text-base hover:text-gray-200">
                  <span class="px-3 py-0.5 text-white text-xs font-semibold leading-5 tracking-wide bg-indigo-600 rounded-full">{{ $t('hero.portfolio') }}</span>
                  <span class="ml-4 text-xs">{{$t('hero.checkItOut')}}</span>
                ...
                <h1 class="mt-4 text-3xl tracking-tight font-extrabold text-white sm:mt-5 sm:leading-none lg:mt-6 lg:text-3xl xl:text-3xl">
                  <span class="md:block">About Lloyd</span>
                  <div>
